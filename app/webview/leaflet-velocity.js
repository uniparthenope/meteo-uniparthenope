...
var Windy = function Windy(params) {
	...
	var buildGrid = function buildGrid(data, callback) {
		...
		lambda = header.lo1;
		phi = header.la1; // the grid's origin (e.g., 0.0E, 90.0N)

		delta_gamma = header.dx;
		delta_phi = header.dy; // distance between grid points (e.g., 2.5 deg lon, 2.5 deg lat)
		...
		// Scan mode 0 assumed. Longitude increases from lambda, and latitude decreases from phi.
		// http://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_table3-4.shtml
		grid = [];
		var p = 0;
		var isContinuous = Math.floor(ni * delta_gamma) >= 360;
	...
	var interpolate = function interpolate(lam, ph) {

		if (!grid) return null;

		var i = floorMod(lam - lambda, 360) / delta_gamma; // calculate longitude index in wrapped range [0, 360)
		var j = (phi - ph) / delta_phi; // calculate latitude index in direction +90 to -90
  ...
  */
	var distort = function distort(projection, lam, ph, x, y, scale, wind, windy) {
		var u = wind[0] * scale;
		var v = wind[1] * scale;
		var d = distortion(projection, lam, ph, x, y, windy);
	...
	var distortion = function distortion(projection, lam, ph, x, y, windy) {
		var tao = 2 * Math.PI;
		var H = Math.pow(10, -5.2);
		var hlam = lam < 0 ? H : -H;
		var hph = ph < 0 ? H : -H;

		var plam = project(ph, lam + hlam, windy);
		var pph = project(ph + hph, lam, windy);

		// Meridian scale factor (see Snyder, equation 4-3), where R = 1. This handles issue where length of 1º lam
		// changes depending on ph. Without this, there is a pinching effect at the poles.
		var k = Math.cos(ph / 360 * tao);
		return [(plam[0] - x) / hlam / k, (plam[1] - y) / hlam / k, (pph[0] - x) / hph, (pph[1] - y) / hph];
	};
			...
			for (var y = bounds.y; y <= bounds.yMax; y += 2) {
				...
				if (coord) {
					var lam = coord[0],
					    ph = coord[1];
					if (isFinite(lam)) {
						var wind = grid.interpolate(lam, ph);
						if (wind) {
							wind = distort(projection, lam, ph, x, y, velocityScale, wind, extent);
